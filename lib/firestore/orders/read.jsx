...
export function useOrder({ id }) {
	...
	const { data, error } = useSWRSubscription(['orders', id], ([path, id], { next }) => {
...
export function useOrders({ uid }) {
	const { data, error } = useSWRSubscription(['orders', uid], ([path, uid], { next }) => {
		const ref = query(
			collection(db, path),
			where('uid', '==', uid),
			...
			const unsub = onSnapshot(
				...
				(snapshot) =>
					next(null, {
						...
						lastSnapDoc:
							snapshot.docs.length === 0 ? null : snapshot.docs[snapshot.docs.length - 1],
					}),
				(err) => next(err, null),
			)
			return () => unsub()
